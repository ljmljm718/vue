...
export const DeviceCategoryApi = {
  ...
  getDeviceMonitorUnit: async (categoryId: string, deviceMonitor: string) => {
    return await request.get({
      url:
        `/agriculture/device-category/getDeviceMonitorUnit?categoryId=` +
        categoryId +
        `&deviceMonitor=` +
        deviceMonitor
    });
  ...
  getDeviceCategoryMonitorListByDeviceId: async (deviceId) => {
    return await request.get({
      url: `/agriculture/device-category/monitor/list-by-device-id?deviceId=` + deviceId
    });
  }
};
