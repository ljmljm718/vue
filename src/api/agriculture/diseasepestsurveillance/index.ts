...
export const DiseasePestSurveillanceApi = {
  ...
  exportDiseasePestSurveillance: async (params) => {
    return await request.download({
      url: `/agriculture/disease-pest-surveillance/export-excel`,
      params
    });
  },
  ...
  pyDiseasePestSurveillance: async (params) => {
    return await request.get({
      url: `/agriculture/identification-result/main-id-by-python?id=` + params
    });
  },
  // python自动识别病害接口并添加
  pyCreateDiseasePestSurveillance: async (data) => {
    return await request.post({
      url: `/agriculture/identification-result/identification-data-add`,
      data
    });
  }
};
