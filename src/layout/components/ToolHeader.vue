      ...
      >
        ...
        <div class="h-full flex items-center">
          ...
          <el-drawer
            ...
            direction="rtl"
          >
            <el-table data={storageList.value} size="small" stripe class="shadow-lg rounded-lg py-1" style="border: 1px solid #25252525;">
              <el-table-column label="表单名称" prop="name" />
              <el-table-column label="表单Id" prop="formId" />
              <el-table-column label="修改时间" formatter={(row) => {
                return <span>{row.timeStamp ? formatTime(row.timeStamp, 'yyyy-MM-dd HH:mm:ss') : '----'}</span>
              }} />
              <el-table-column label="操作" formatter={(row) => {
                return (
                  <>
                    <router-link
                    ...
                    <el-button link type={'danger'} onClick={() => handleDeleteStorageForm(row)}>删除</el-button>
                  </>
                )
              }} />
            </el-table>
