...
import VillageProcessingRecordsForm from './VillageProcessingRecordsForm.vue';

/** 加工记录 列表 */
defineOptions({ name: 'VillageProcessingRecords' });

const message = useMessage(); // 消息弹窗
const { t } = useI18n(); // 国际化

const loading = ref(true); // 列表的加载中
const list = ref<VillageProcessingRecordsVO[]>([]); // 列表的数据
...
const getList = async () => {
  loading.value = true;
  try {
    const data = await VillageProcessingRecordsApi.getVillageProcessingRecordsPage(queryParams);
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await VillageProcessingRecordsApi.deleteVillageProcessingRecords(id);
...
const handleExport = async () => {
  try {
    ...
    const data = await VillageProcessingRecordsApi.exportVillageProcessingRecords(queryParams);
    download.excel(data, '加工记录.xls');
  } catch {
  } finally {
...
<template>
  ...
  >
    <div class="w-full flex justify-between items-center">
      <div class="flex items-center">
        <!-- 一级标题名字 -->
        <h1 class="m-0 text-[#333] font-bold text-[18px]">加工记录</h1>
      ...
      <div class="flex items-center">
        ...
        <el-button @click="handleExport" v-hasPermi="['digital:village-processing-records:export']">
    ...
    >
      ...
      -->
      <el-form-item label="品种名称" prop="product">
        <el-input
      ...
      <el-form-item label="加工时间" prop="createTime">
        <el-date-picker
  ...
  <!-- 页面使用的组件复制在下面 -->
  <!-- 表单弹窗：添加/修改 -->
  <VillageProcessingRecordsForm ref="formRef" @success="getList" />
</template>
