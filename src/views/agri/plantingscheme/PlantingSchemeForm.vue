...
/** AI种植方案 表单 */
defineOptions({ name: 'PlantingSchemeForm' });
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await PlantingSchemeApi.getPlantingScheme(id);
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true;
  try {
    const data = formData.value as unknown as PlantingSchemeVO;
    if (formType.value === 'create') {
      await PlantingSchemeApi.createPlantingScheme(data);
      message.success(t('common.createSuccess'));
    } else {
      await PlantingSchemeApi.updatePlantingScheme(data);
