      ...
      <el-table-column label="操作" align="center">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['agri:user-table:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['agri:user-table:delete']"
          >
            删除
          </el-button>
        </template>
...
const getList = async () => {
  loading.value = true
  try {
    const data = await UserTableApi.getUserTablePage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await UserTableApi.deleteUserTable(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await UserTableApi.exportUserTable(queryParams)
    download.excel(data, '数农门户.xls')
  } catch {
  } finally {
