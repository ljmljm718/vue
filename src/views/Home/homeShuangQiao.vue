      ...
      >
        ...
        <div v-if="item.title === '设备总数' || item.title === '预警数量'">
          <div class="flex items-center pt-2 h-[2.5rem] px-2" style="font-weight: 600">
            ...
            <span
              v-if="item.title === '设备总数'"
              style="padding-left: 2rem"
              @click="$router.push('/internetMonitor/device/deviceView')"
              >台</span
            >
            <span
              v-if="item.title === '预警数量'"
              style="padding-left: 2rem"
              @click="$router.push('/internetMonitor/warn/agri-warning-record')"
              >个</span
            >
          </div>
        </div>
        <div v-else>
          <div style="font-weight: 600" class="h-[1rem] p-1 pt-2 flex items-center">
            <span
              ...
              >在线</span
            >
            <div style="padding-left: 4rem" class="flex space-x-2 items-center">
        ...
        <el-skeleton :loading="loading" animated>
          ...
          <div class="p-1">
            <div
              ...
              style="
                font-weight: 600;
                background: linear-gradient(to right, #79cefe00, #79cefea0, #79cefe00);
              "
          ...
          <div class="flex pl-7 title-icon justify-between items-center">
            ...
            <div class="selector-wrapper" @click="(e) => e.stopPropagation()">
              <select @change="handleSelectorChangeX">
                <option :value="item.id" v-for="(item, index) in optionsX" :key="index"
                  >{{ item.name }}
                </option>
              </select>
            </div>
          ...
          <div class="grid grid-cols-3 gap-3">
            <div v-for="item in deviceCheckList" :key="item.deviceId">
              ...
              >
                ...
                <span style="padding-left: 0.3rem">{{ item.sumNum }}</span>
              </div>
              <div class="grid grid-cols-2 mt-2">
                <div class="flex" style="color: #0fc87c">
                  <span>已巡检</span>
                  <span style="padding-left: 0.3rem">{{ item.finishCheckNum }}</span>
                </div>
                <div class="flex" style="color: #ff0000">
                  <span>未巡检</span>
                  <span style="padding-left: 0.3rem">{{ item.unFinishCheckNum }}</span>
                </div>
              </div>
              <div></div>
            </div>
