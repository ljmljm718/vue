...
const handleShortcutDaysChange = (e) => {
  ...
  if (e == '本周') {
    ...
    bugTimeRange.value.push(
      `${new Date(getDaysOfWeek(day)[0]).getFullYear()}-${new Date(getDaysOfWeek(day)[0]).getMonth() + 1}-${new Date(
        getDaysOfWeek(day)[0]
      ).getDate()}`
    );
    bugTimeRange.value.push(
      `${new Date(getDaysOfWeek(day)[6]).getFullYear()}-${new Date(getDaysOfWeek(day)[6]).getMonth() + 1}-${new Date(
        getDaysOfWeek(day)[6]
      ).getDate()}`
    );
  ...
  <div class="flex space-x-[.5rem] domDiv">
    ...
    <div style="width: calc(100% - 12.5rem)">
      ...
      <div class="flex justify-between box-border pr-[.5rem]">
        <div class="grow space-y-[1rem] flex flex-col box-border" style="max-width: calc(100% - 24.5rem)">
            ...
            <div class="space-y-[1rem] pr-.5rem pb-.5rem">
              <el-card>
                ...
                <div class="flex justify-center space-x-1rem">
                  <div :class="`max-w-70rem insect-disease1 grid grid-cols-1 xl:grid-cols-2 gap-2 box-border grow py-3`">
                    <div>
                      ...
                      <el-scrollbar class="left-scrollbar" height="14rem">
                        <div class="flex items-center space-x-3 mb-1 p-2" v-for="(item, index) in insectList" :key="item.id">
                          <div
                            class="w-1.6rem h-1.6rem rounded-full flex justify-center items-center text-[#fff] text-[.8rem]"
                        ...
                        <div class="h-12rem w-full flex items-center justify-center" v-if="insectList.length === 0">暂无数据</div>
                      </el-scrollbar>
                    </div>
                    <div>
                      <div class="h-2rem flex items-center px-1">
                      ...
                      <el-scrollbar height="14rem">
                        <div v-for="(item, index) in diseaseList" :key="item.id" class="flex items-center space-x-3 mb-1 p-2">
                          <div
                            class="w-1.6rem h-1.6rem rounded-full flex justify-center items-center text-[#fff] text-[.8rem]"
                            :style="`background-color: ${index < 3 ? '#009688' : '#cccccc'}`"
                          >
                        ...
                        <div class="h-12rem w-full flex items-center justify-center" v-if="diseaseList.length === 0">
                          暂无数据
                        </div>
                      </el-scrollbar>
                    </div>
              ...
              <el-card>
                <div
                  class="2xl:flex 2xl:justify-between 2xl:items-center 2xl:flex-wrap xl:flex xl:justify-between xl:items-center xl:flex-wrap lg:grid lg:grid-cols-1 gap-4 space-y-2 py-10px"
                >
                  <div class="title-frame">病害趋势分析</div>
                  <div>
                    <el-date-picker
                      :style="`width:${windWidth < 1200 ? 'auto' : 'auto'} `"
        ...
        <div class="w-24rem">
          <div class="flex flex-col space-y-[1rem]">
            <el-card class="2xl:h-725px xl:h-800px md:h-1330px sm:h-1410px">
              <div height="calc(100vh - 440px)">
                ...
                <div class="flex justify-between space-x-2 py-2">
                  <el-select v-model="snapShotDevice" placeholder="请选择" clearable @change="selectChange">
                    <el-option v-for="item in snapShotDeviceOptions" :key="item.id" :label="item.deviceName" :value="item.id" />
                  </el-select>
                  <el-date-picker v-model="snapShotTime" @change="snapShotChange" type="date" placeholder="选择日期" />
                </div>
                ...
                <div v-if="snapDevice" class="rounded-1 flex justify-evenly items-center bg-#00968810 p-2 my-2">
                  <div>
                    ...
                    <div class="text-center color-[#3a3a3a] text-15px mb-7px" style="font-weight: 600">
                      {{ snapDevice?.monitorSpecies }}
                    </div>
                  </div>
                  <div>
                ...
                <div class="h-3rem w-full my-2 flex items-center bg-#00968810 justify-center" v-if="!snapDevice">暂无数据</div>
                ...
                <el-scrollbar
                  :height="`${windWidth < 1200 ? '250px' : '100px'} `"
            ...
            <el-card class="xl:h-390px md:h-640px sm:h-650px 2xl:h-380px">
              ...
              <el-scrollbar :height="`${windWidth < 1200 ? '520px' : '250px'} `">
                <div class="p-3 box-border" v-loading="preWarnLoading">
                  <div class="py-1rem" style="border-top: 1px solid #99999980" v-for="item in preWarnList" :key="item.id">
                    <div class="line-clamp-1 mb-2 cursor-pointer font-bold text-[.9rem]" :title="item.lowMsg">
                      {{ item.lowMsg }}
                    </div>
                  ...
                  <div class="h-10rem w-full flex items-center justify-center" v-if="preWarnList.length === 0">暂无数据</div>
                </div>
              </el-scrollbar>
            </el-card>
          </div>
