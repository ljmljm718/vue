  ...
  <el-tabs v-model="activeName" @tab-click="changeTab">
    ...
    <el-tab-pane label="传感设备预警" name="sensorTab">
      ...
      <ContentWrap>
        ...
        >
          <!-- 表单内容 -->
          <div class="lg:col-span-4 xl:col-span-5 2xl:col-span-6 lg:border-r lg:border-r-solid lg:border-[#E5E5E5] grid grid-cols-1 xl:grid-cols-2 2xl:grid-cols-4 gap-2 lg:pr-20px">
      ...
      <ContentWrap>
        ...
        <div class="mt-[20px]">
          <!-- 卡片形式 -->
          <div v-show="listType === 'card'" class="grid sm:grid-cols-2 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3 gap-4">
            <div v-for="item in list" :key="item.id" class="card-item">
              ...
              <div class="h-[22px] flex">
                ...
                <dict-tag class="ml-20px" :type="DICT_TYPE.AGRI_WARN_LEVEL" :value="item.warnLevel"/>
                <dict-tag class="ml-10px" :type="DICT_TYPE.AGRI_MONITOR_TYPE" :value="item.warnType"/>
              </div>
              <!-- 第二行 -->
              ...
              <div class="warn-popover">
                ...
                {{ item.warnInfo }}!
                <!-- 旋转10乘10的正方形区域45度 -->
                <div style="position: absolute; top: -5px; left: 60px; background-image: linear-gradient(to bottom right, hsla(4, 96%, 45%, 0.08), transparent); width: 10px; height: 10px; transform: rotate(45deg)"></div>
              </div>

              <!-- 第三行 -->
              <!-- grid布局，两行两列，已处理显示四个格子，未处理只显示第1个格子 -->
              <div class="grid grid-cols-2 grid-rows-2 gap-1 mt-15px text-[#999999] text-[14px] font-['AlibabaPuHuiTi']" v-show="'1' === item.warnStatus">
                <div class="">
                  预警状态: <span class="text-[#009688]">已处理</span>
                </div>
                <div>
                  处理人: {{ item.dealPerson }}
                </div>
                ...
                <div>
                  处理时间: {{ timeFormat(item.dealTime) }}
                </div>
              </div>
              <div class="grid grid-cols-2 grid-rows-2 gap-1 mt-15px text-[#999999] text-[14px] font-['AlibabaPuHuiTi']" v-show="'0' === item.warnStatus">
                <div class="">
                  预警状态: <span class="text-[#FF8500]">未处理</span>
                </div>
                <div></div>
                <div></div>
                <div></div>
          ...
          <div v-show="listType === 'list'">
            <el-table :data="list">
              ...
              <el-table-column align="center" prop="warnInfo" label="预警信息" class-name="text-[#E31205]" label-class-name="text-zinc-500"/>
    ...
    <el-tab-pane label="监控设备预警" name="monitorTab">
      ...
      <ContentWrap>
        ...
        >
          <!-- 表单内容 -->
          <div class="grid grid-cols-1 lg:grid-cols-2 2xl:grid-cols-4 gap-2 md:col-span-3 lg:col-span-4 xl:col-span-5 2xl:col-span-6 md:border-r md:border-r-solid md:border-[#E5E5E5] md:pr-20px">
      ...
      <ContentWrap>
        ...
        <div class="mt-[20px]">
          <!-- 卡片形式 -->
          <div v-show="listTypeMonitor === 'card'" class="grid grid-cols-8 lg:grid-cols-3 2xl:grid-cols-2 gap-3 text-[12px] 2xl:text-[14px] text-[#999999]">
            <!-- 预览区 -->
            <div class="col-span-7 lg:col-span-2 2xl:col-span-1 rounded-md bg-[#F5F5F5] shadow-md">
              <div class="relative">
                <el-image
                  ...
                  class="w-full"
                />
                <div v-show="currentItem.videoLink" @click="openVideoMonitor(currentItem.videoLink)" class="absolute bg-black opacity-50 w-[40px] h-[40px] bottom-[11px] right-[110px] rounded text-center leading-[40px] hover:cursor-pointer">
                  <el-icon color="#FFFFFF" size="16px"><VideoCamera /></el-icon>
                </div>
                <div class="absolute bg-black opacity-50 w-[40px] h-[40px] bottom-[11px] right-[60px] rounded text-center leading-[40px] hover:cursor-pointer" @click="openFormMonitor('update', currentItem.id)">
                  <el-icon color="#FFFFFF" size="16px"><Edit /></el-icon>
                </div>
                <div class="absolute bg-black opacity-50 w-[40px] h-[40px] bottom-[11px] right-[10px] rounded text-center leading-[40px] hover:cursor-pointer" @click="handleDeleteMonitor(currentItem.id)">
                  <el-icon color="#FFFFFF" size="16px"><Delete /></el-icon>
                </div>
              </div>
              <div class="grid grid-cols-3 gap-1 my-10px px-3">
                <div>基地名称: <span class="text-[#666666]">{{ currentItem.monitoringBaseName }}</span></div>
                <div>地块名称: <span class="text-[#666666]">{{ currentItem.monitoringPlotName }}</span></div>
                <div>设备名称: <span class="text-[#666666]">{{ currentItem.deviceName }}</span></div>
                <div>事件类型: <span class="text-[#666666]">{{ currentItem.noticeEvent }}</span></div>
                <div>消息内容: <span class="text-[#666666]">{{ currentItem.remarks }}</span></div>
                <div>记录时间: <span class="text-[#666666]">{{ timeFormat(currentItem.recordTime) }}</span></div>
              </div>
            </div>
            <!-- 卡片列表区 -->
            <div class="col-span-1 grid grid-cols-1 grid-rows-6 lg:grid-cols-2 lg:grid-rows-3 2xl:grid-cols-3 2xl:grid-rows-2 gap-3 rounded">
              <div class="bg-[#F5F5F5] cursor-pointer 2xl:grid 2xl:grid-rows-4 shadow-md rounded-md" v-for="item in listMonitor" :key="item.id" @click="changCurrentItem(item)">
                <el-image
                  lazy
                  :src="item.captured"
                  preview-teleported
                ...
                />
                <div class="grid grid-cols-1 2xl:row-span-1 2xl:gap-1 2xl:mt-[10px] text-[4px] lg:text-[8px] xl:text-[10px] 2xl:text-[14px] ml-2px mb-2px px-3">
                  <div>设备名称: <span class="text-[#666666]">{{ item.deviceName }}</span></div>
                  <div>记录时间: <span class="text-[#666666]">{{ timeFormat(item.recordTime) }}</span></div>
                </div>
              </div>
            </div>    
          </div>
            ...
            <el-table :data="listMonitor">
              ...
              <el-table-column align="center" label="操作" fixed="right" width="280px">
                <template #default="scope">
                  ...
                  <el-button
                    @click="openFormMonitor('update', scope.row.id)"
                  >
                    编辑
                  </el-button>
                  <el-button
...
import { vi } from 'node_modules/element-plus/es/locale'
...
const getListMonitor = async () => {
  ...
  try {
    const data = await MonitoringEquipmentNoticeApi.getMonitoringEquipmentNoticePage(queryParamsMonitor)
