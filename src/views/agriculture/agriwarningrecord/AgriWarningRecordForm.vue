...
const handlePurchaseOrderChange = async (order: EquipmentDataVO) => {
  ...
  //根据设备id查询对应的监测类型
  const deviceMonitorList = await DeviceCategoryApi.getDeviceCategoryMonitorListByDeviceId(
    order[0].deviceKind
  );
  const monitorList = [];
  const unitList = [];
  deviceMonitorList.forEach((item) => {
    monitorList.push({ value: item.monitor, label: item.monitor });
    unitList.push({ value: item.unit, label: item.unit });
  });
  options.value = monitorList;
