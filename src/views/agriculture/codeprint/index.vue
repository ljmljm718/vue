...
import CodeSendingInfoForm from '../codesendinginfo/CodeSendingInfoForm.vue';
import QRCode from 'qrcode';

/** 发码记录 列表 */
defineOptions({ name: 'CodePrint' });
  ...
  >
    <div class="w-full flex justify-between items-center">
      <div class="flex items-center">
        <!-- 一级标题名字 todo替换成菜单名称-->
        <h1 class="m-0 text-[#333] font-bold text-[18px]">源码打印管理</h1>
      ...
      >
        ...
        <el-table-column label="批次号" align="center" prop="batchNum">
          <template #default="scope">
            <span>
              {{
                scope.row.batchNum != undefined && scope.row.batchNum != null
                  ? scope.row.batchNum
                  : '无'
              }}
            </span>
