...
<template>
  <div class="w-full h-full overflow-auto text-[#000]">
    ...
    <img :src="dataInfo?.headerImg" class="w-full h-18rem object-cover" />
    <div class="linear-bg py-[1.6rem] flex flex-col items-center">
      ...
      <div class="relative mt-2rem w-[90%]">
        <div class="rect-bg aspect-3 box-border p-2rem space-y-.4rem">
          <div class="flex items-start justify-between">
            <div class="w-4.5rem">产品:</div>
            <div class="value-right">{{ dataInfo?.brandName }}</div>
          </div>
          <div class="flex items-start justify-between">
            <div class="w-4.5rem">生产商:</div>
            <div class="value-right">
              {{ getLabelById(dataInfo?.mfrsId) ?? dataInfo?.producer }}
            </div>
          </div>
          <div class="flex items-start justify-between">
          ...
          <div class="flex items-start justify-between">
            ...
            <div class="value-right">
              {{
                dataInfo?.productionTime
                  ? dayjs(dataInfo.productionTime).format('YYYY-MM-DD HH:mm:ss')
                  : ''
              }}
            </div>
          </div>
      ...
      <div class="relative mt-1.8rem w-[90%]">
        <div class="rect-bg aspect-3 pt-1.4rem px-1.3rem pb-.6rem" style="text-indent: 2rem">
      ...
      <div v-if="resourceInfoList.length > 0" class="w-90% flex flex-col space-y-3 mt-1.6rem">
        ...
        >
          <div class="flex flex-col items-center w-2rem h-full relative top-1rem">
            ...
            <div class="w-1px h-2rem mt-.4rem" style="border-right: 1px dashed #009688"></div>
          </div>
          <div
            class="grow shadow-md h-3.6rem flex space-x-1rem items-center box-border px-1rem"
            style="border: 1px solid #e1e1e1"
          >
            <div :class="`w-2rem h-2rem rounded-full ${item.icon}`"></div>
            <div>
              <div>{{ item.defineName }}</div>
              <div class="text-#999 text-.8rem">
                {{ dayjs(item.recordTime).format('YYYY-MM-DD HH:mm:ss') }}
              </div>
            </div>
          </div>
        </div>
      </div>
      <div v-else class="w-90% h-[10rem] mt-1.6rem no-data text-center tracking-widest">
        暂无数据
      </div>
    </div>
    <div class="py-[1.6rem] flex flex-col items-center">
