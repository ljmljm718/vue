<template>
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="主表" prop="parkId">
        <el-input v-model="formData.parkId" placeholder="请输入主表" />
      </el-form-item>
...
/** 地块基本信息 表单 */
defineOptions({ name: 'ParkDetailForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await ParkDetailApi.getParkDetail(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as ParkDetailVO
    if (formType.value === 'create') {
      await ParkDetailApi.createParkDetail(data)
      message.success(t('common.createSuccess'))
    } else {
      await ParkDetailApi.updateParkDetail(data)
