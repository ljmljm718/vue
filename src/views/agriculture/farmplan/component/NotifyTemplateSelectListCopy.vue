...
defineOptions({ name: 'NotifyTemplateSelectListCopy' });

const message = useMessage(); // 消息弹窗

const planId = ref(); // 传入的农事计划id
...
const handleBindTemplate = async () => {
  loading.value = true;
  try {
    const temp = { planId: planId.value, templateIds: templateIds.value };
    const data = temp as any;
    await NotifyTemplateRuleApi.FarmPlanBindTemplate(data);
