<template>
  ...
  >
    <el-table :data="formData" class="-mt-10px">
      <el-table-column label="序号" type="index" width="100" />
       <el-table-column label="规格流水号" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.specsNumber`" :rules="formRules.specsNumber" class="mb-0px!">
            <el-input v-model="row.specsNumber" placeholder="请输入规格流水号" />
          </el-form-item>
        </template>
      </el-table-column>
      <el-table-column label="规则类型" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.ruleType`" :rules="formRules.ruleType" class="mb-0px!">
            <el-select v-model="row.ruleType" placeholder="请选择规则类型">
      ...
      <el-table-column label="单价" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.singlePrice`" :rules="formRules.singlePrice" class="mb-0px!">
            <el-input v-model="row.singlePrice" placeholder="请输入单价" />
          </el-form-item>
        </template>
      </el-table-column>
      <el-table-column label="份数" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.servingNumber`" :rules="formRules.servingNumber" class="mb-0px!">
            <el-input v-model="row.servingNumber" placeholder="请输入份数" />
          </el-form-item>
        </template>
      </el-table-column>
      <el-table-column label="每份数量" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.servingAmount`" :rules="formRules.servingAmount" class="mb-0px!">
            <el-input v-model="row.servingAmount" placeholder="请输入每份数量" />
          </el-form-item>
        </template>
      </el-table-column>
  ...
  <el-row justify="center" class="mt-3">
    <el-button @click="handleAdd" round>+ 添加认养规则规格</el-button>
  </el-row>
</template>
