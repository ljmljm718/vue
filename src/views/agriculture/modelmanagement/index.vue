  ...
  <ContentWrap>
    ...
    <div class="flex">
      <el-form :model="queryParams" ref="queryFormRef" :inline="true" label-width="68px" :style="{ height: '22px' }"
        ...
        <el-form-item class="w-full h-full">
          <!-- <el-button
            type="success"
            plain
            @click="handleExport"
    ...
    <div v-if="showType === 'card'" class="mt-3 mb-3 grid gap-3 grid-cols-3  2xl:grid-cols-4 changecols">
      <div v-for="item, index in list" :key="index" class="p-5 bg-[#f5f5f5]">
        ...
        <div class="mt-15px grid  grid-cols-4">
          <el-button style="font-size:12px; " type="primary" 
            @click="router.push(`/growth_monitor/model-monitor-indicator?modelId=${item.id}&belongVarietyId=${item.belongVarietyId}`)">
            监测指标
          </el-button>
          <el-button style="font-size:12px; "  @click="openForm('detail', item.id)" 
                     v-hasPermi="['agriculture:model-management:update']">
            详情
          </el-button>
          <el-button style="font-size:12px; "  @click="openForm('update', item.id)"
            v-hasPermi="['agriculture:model-management:update']">
            编辑
          </el-button>
          <el-button style="font-size:12px; "  @click="handleDelete(item.id)"  
