...
import { IrrigationAreaApi } from '@/api/agriculture/irrigationarea';
...
const props = defineProps<{
  irrigationId: undefined; // 主表（主表的关联字段）
}>();
...
const getList = async () => {
  loading.value = true;
  try {
    list.value = await IrrigationAreaApi.getParkDetailListByIaId(props.irrigationId);
  } finally {
    loading.value = false;
  }
};
