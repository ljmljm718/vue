<template>
    ...
    <div class="bg-white p-3">
      ...
      <div class="mt-3 flex space-x-2">
        <div class="w-[10rem] flex flex-col" style="border-right: 1px solid #66666690">
          <div
            v-for="(item, index) in growthPeriod"
            :key="index"
            :class="[
        ...
        <div class="p-3 grow">
          ...
          <div class="flex justify-between mt-3 h-10">
            ...
            <el-button
              class="!text-white !bg-[#009688]"
              v-hasPermi="['agri:crop-growth-new:create']"
              @click="openSubDeviceForm(cropid)"
            >
              <Icon icon="ep:plus" class="mr-5px" />新增
            </el-button>
          </div>
              ...
              <el-table-column label="操作" align="center" width="230px">
                <template #default="scope">
                  <el-button
                    link
                    type="primary"
                    @click="EditSubDeviceForm(scope.row.id)"
                    v-hasPermi="['agri:crop-growth-new:update']"
                  >
                    编辑
                  </el-button>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <CropGrowthNewForm ref="formRef" @success="getCropGrowthList" />
...
const getCropGrowthList = async (id = route.query.cropId) => {
  ...
  const data = await getGrowthCycleListApi({ cropId: id })
  growthList.value = data
...
getCropGrowthList(route.query.cropId)

// ------------获得事项列表------------
const EventList = ref<any[]>([])
const getEventList = async (id = cropid.value, numPage = 1, size = 10) => {
...
const selectGrowth = (key) => {
  ...
  cropid.value = Number(key)
  console.log('cropid', cropid.value)
...
const openSubDeviceForm = (id) => {
  ...
  console.log('生长期的id', cropid.value)
  getEventList(cropid.value, 1, 10)
}
//-----------事项的编辑-----------------
