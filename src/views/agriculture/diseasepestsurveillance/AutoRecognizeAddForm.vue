...
defineOptions({ name: 'AutoRecognizeAddForm' });
const { t } = useI18n(); // 国际化
...
watch(
  ...
  (value) => {
    if (value !== undefined && value.size !== 0) {
      ...
      baseForm.demoList = Array.from(map, ([name, count]) => ({
        name: name,
        quantity: count
      }));
      console.log('Auto baseForm.demoList', baseForm.demoList);
    }
  }
);
...
const saveInfo = (index) => {
  ...
  //   alert('请填写完整信息')
  //   return
  if (baseForm.demoList[index].name == '' || baseForm.demoList[index].quantity == '') {
    alert('请填写完整信息');
    return;
  }
...
const handleSubmit = async () => {
  ...
  const data = baseForm.demoList.map(({ isEditing, ...rest }) => {
    return {
      ...rest
    };
  });
  ...
  try {
    if (data.length > 0) {
      reqData.mainTableId = data[0].mainTableId;
      reqData.mainTableImg = props.imgId;
      // 将虫害列表转换为对象（Map）
      reqData.insectMap = computed(() => {
        return data.reduce((acc, list) => {
          acc[list.name] = list.quantity;
          return acc;
        }, {});
      }).value;
      console.log('reqData', reqData);
    }
    await DiseasePestSurveillanceApi.pyCreateDiseasePestSurveillance(reqData);
...
const closeDialog = () => {
  ...
  console.log('close baseForm.demoList', baseForm.demoList);
  dialogVisible.value = false;
};
...
<template>
  <Dialog title="自动识别补充" v-model="dialogVisible" :width="1000" :height="1000">
    <div class="flex w-full flex justify-center items-center">
      <img :src="props.imgId" alt="" class="w-1/2 h-auto object-contain" />
    </div>
    ...
    >
      ...
      >
          ...
          <template #default="scope">
            ...
            >
              <el-select
                ...
                class="!w-280px relative top-[.5rem]"
              >
                <el-option
        ...
        <!--              v-if="scope.row.isEditing"-->
        <!--            >-->
        <!--              <el-input type="number" class="relative top-[.5rem]" v-model="scope.row.density" placeholder="请输入密度" oninput="value=value.replace(/[^\d.]/g,'')"/>-->
        <!--            </el-form-item>-->
        <!--          </template>-->
        <!--        </el-table-column>-->
            ...
            <el-form-item>
              <div class="relative top-[.5rem]" style="width: 100%; text-align: center">
                ...
                <el-button
                  link
                  type="primary"
                  @click="$router.push('/dict/type/data/agri_disease_name')"
                >
                  配置
                </el-button>
