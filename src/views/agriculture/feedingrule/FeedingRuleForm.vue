    ...
    >
      ...
      <!-- <div class= 'flex'> -->
        <el-form-item label="重量(克)" prop="weight" >
          <el-input-number class='!w-100%' v-model="formData.weight" placeholder="请输入重量"  />
        </el-form-item>
      ...
      <el-form-item label="策略" prop="strategy">
        <el-input v-model="formData.strategy" type="textarea" placeholder="请输入策略" />
      </el-form-item>
    </el-form>
    <template #footer>
...
/** 投喂规则 表单 */
defineOptions({ name: 'FeedingRuleForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await FeedingRuleApi.getFeedingRule(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as FeedingRuleVO
    if (formType.value === 'create') {
      await FeedingRuleApi.createFeedingRule(data)
      message.success(t('common.createSuccess'))
    } else {
      await FeedingRuleApi.updateFeedingRule(data)
