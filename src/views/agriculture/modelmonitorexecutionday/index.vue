<template>
  <ContentWrap>
    ...
    >
      ...
      <el-form-item label="检测日期" prop="monitorDate">
        <el-date-picker
      ...
      <el-form-item>
        ...
        <el-button
          ...
          v-hasPermi="['agriculture:model-monitor-execution-day:create']"
        >
        ...
        <el-button
          ...
          v-hasPermi="['agriculture:model-monitor-execution-day:export']"
        >
      ...
      <el-table-column label="操作" align="center">
        <template #default="scope">
          <el-button
            ...
            v-hasPermi="['agriculture:model-monitor-execution-day:update']"
          >
            编辑
          </el-button>
          <el-button
            ...
            v-hasPermi="['agriculture:model-monitor-execution-day:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <ModelMonitorExecutionDayForm ref="formRef" @success="getList" />
</template>

<script setup lang="ts">
import { dateFormatter } from '@/utils/formatTime'
import download from '@/utils/download'
import { ModelMonitorExecutionDayApi, ModelMonitorExecutionDayVO } from '@/api/agriculture/modelmonitorexecutionday'
import ModelMonitorExecutionDayForm from './ModelMonitorExecutionDayForm.vue'

/** 监测执行日健康 列表 */
defineOptions({ name: 'ModelMonitorExecutionDay' })

const message = useMessage() // 消息弹窗
const { t } = useI18n() // 国际化

const loading = ref(true) // 列表的加载中
const list = ref<ModelMonitorExecutionDayVO[]>([]) // 列表的数据
...
const getList = async () => {
  loading.value = true
  try {
    const data = await ModelMonitorExecutionDayApi.getModelMonitorExecutionDayPage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await ModelMonitorExecutionDayApi.deleteModelMonitorExecutionDay(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await ModelMonitorExecutionDayApi.exportModelMonitorExecutionDay(queryParams)
    download.excel(data, '监测执行日健康.xls')
  } catch {
  } finally {
