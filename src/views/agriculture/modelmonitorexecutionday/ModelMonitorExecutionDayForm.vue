...
/** 监测执行日健康 表单 */
defineOptions({ name: 'ModelMonitorExecutionDayForm' })
...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await ModelMonitorExecutionDayApi.getModelMonitorExecutionDay(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as ModelMonitorExecutionDayVO
    if (formType.value === 'create') {
      await ModelMonitorExecutionDayApi.createModelMonitorExecutionDay(data)
      message.success(t('common.createSuccess'))
    } else {
      await ModelMonitorExecutionDayApi.updateModelMonitorExecutionDay(data)
