...
const handleConfirm = async () => {
  ...
  const cropGrowthNew = selectedData.value.map((item) => toRaw(item)),
    basePlot: any[] = []
...
<template>
  <Dialog title="批量添加生长周期" v-model="dialogVisible" width="70vw">
    <ContentWrap>
      <div class="flex justify-between w-full">
        ...
        <div class="flex items-center space-x-3">
          <div>
            <span>已选择</span>
            <span :style="`color: ${selectedData.length > 0 ? '#009688' : '#da4e52'};padding: 0 .3rem;`">
              {{ selectedData.length }}
            </span>
            <span>条，新增到</span>
            <span :style="`color: ${selectedPlot.length > 0 ? '#009688' : '#da4e52'};padding: 0 .3rem;`">
              {{ selectedPlot.length }}
            </span>
            <span>个地块</span>
          </div>
          ...
          <el-button class="!px-5" @click="handleCancel"> 取消 </el-button>
        </div>
      </div>
    </ContentWrap>
          ...
          <template #default="{ node }">
            <div class="w-full flex justify-between items-center">
              ...
              <div class="cell" @click="handleCheckClick(node)" v-show="!node.data.children">
                <label
                  :class="`el-checkbox el-checkbox--small ${selectedPlot.indexOf(node.data.id) !== -1 ? 'is-checked' : ''}`"
                >
                  <span
                    :class="`el-checkbox__input ${selectedPlot.indexOf(node.data.id) !== -1 ? 'is-checked' : ''}`"
                  >
      ...
      <div style="width: calc(100% - 16.8rem)">
        <ContentWrap>
          <div class="flex justify-between flex-wrap">
            ...
            >
              ...
              <el-form-item label="品种名称" prop="monitorType">
                <el-input
            ...
            <div class="">
              ...
              <el-button class="!px-5" @click="resetQuery"> 重置 </el-button>
            </div>
          </div>
        </ContentWrap>
        <ContentWrap class="!mb-0">
          ...
          >
            ...
            <el-table-column label="品类" prop="cropType" />
            <el-table-column label="品种" prop="cropName" />
            <el-table-column label="生长期" prop="growth" width="100">
            ...
            <el-table-column label="周期/天" prop="cycle" width="70" />
            <el-table-column
              label="环境条件"
              width="350"
              prop="envCondition"
            >
              <template #default="scope">
                <el-input
                  v-model="scope.row.envCondition"
                  type="textarea"
                  :rows="3"
                  class="!w-full"
                  @focus="handleFocus(scope.row)"
