<template>
  ...
  >
    <div class="w-full flex justify-between items-center">
      <div class="flex items-center">
        <!-- 一级标题名字 -->
        <h1 class="m-0 text-[#333] dark:text-[#ccc] font-bold text-[18px]">模型配置管理</h1>
        ...
        <el-button
          ...
          v-hasPermi="['bigmodel:big-model-config:create']"
        >
      ...
      <div class="flex items-center">
        ...
        <el-button
          ...
          v-hasPermi="['bigmodel:big-model-config:export']"
        >
    ...
    >
      ...
      <el-form-item label="所属大模型" prop="bigModel">
        <el-select v-model="queryParams.bigModel" placeholder="请选择所属大模型" clearable>
          <el-option
      ...
      <el-form-item label="模型类型" prop="modelType">
        <el-input v-model="queryParams.modelType" placeholder="请输入模型类型" clearable />
      </el-form-item>
    ...
    <div class="w-full mt-[16px]">
      ...
      <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
        <el-table-column label="所属大模型" align="center" prop="bigModel">
          <template #default="scope">
            <dict-tag :type="DICT_TYPE.BIGMODEL_CONFIG_BELONG" :value="scope.row.bigModel" />
          </template>
        </el-table-column>
        ...
        <el-table-column label="嵌入模型" align="center" prop="embeddingModel" width="200" />
        <el-table-column label="鉴权" align="center" prop="authorization" />
        <el-table-column label="索引类型" align="center" prop="indexType" />
        <el-table-column label="向量维度" align="center" prop="embeddingDimension" />
        <el-table-column label="量化方式" align="center" prop="quant" />
        <el-table-column label="片段长度" align="center" prop="chunkLength" />
          ...
          <template #default="scope">
            <div class="flex items-center justify-center">
              <el-button
                ...
                v-hasPermi="['bigmodel:big-model-config:update']"
              >
                详情
              </el-button>
              ...
              <el-button
                ...
                v-hasPermi="['bigmodel:big-model-config:delete']"
              >
                删除
              </el-button>
            </div>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <BigModelConfigForm ref="formRef" @success="getList" />
</template>
...
import BigModelConfigForm from './BigModelConfigForm.vue';
import { DICT_TYPE, getStrDictOptions } from '@/utils/dict';

/** 模型配置 列表 */
defineOptions({ name: 'BigModelConfig' });

const message = useMessage(); // 消息弹窗
const { t } = useI18n(); // 国际化

const loading = ref(true); // 列表的加载中
const list = ref<BigModelConfigVO[]>([]); // 列表的数据
...
const getList = async () => {
  loading.value = true;
  try {
    const data = await BigModelConfigApi.getBigModelConfigPage(queryParams);
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await BigModelConfigApi.deleteBigModelConfig(id);
...
const handleExport = async () => {
  try {
    ...
    const data = await BigModelConfigApi.exportBigModelConfig(queryParams);
    download.excel(data, '模型配置.xls');
  } catch {
  } finally {
