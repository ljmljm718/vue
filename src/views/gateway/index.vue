...
const getGovPolicyDataList = async () => {
  ...
  govPolicyDataList.value = list.map((item) => ({
    ...item,
    year: dayjs(item.upTime).year(),
    date: dayjs(item.upTime).format('MM-DD')
  }));
};
...
const initChinaMap = async () => {
  ...
  myChart.setOption(
    {
      dataRange: {
        // 不显示面板 且 全选series.data并设置背景色
        show: false,
        splitList: [{ start: 2000, end: 2000, color: '#2A8941' }]
      },
      series: [
        {
          type: 'map',
          ...
          label: {
            ...
            overflow: 'break',
            formatter: (item) => {
              // labelMap里的在地图上不显示文字 '璧山区'垂直显示
              const _name = item.name;
              const labelMap = ['九龙坡区', '大渡口区', '渝中区', '沙坪坝区', '江北区', '南岸区'];
              return labelMap.indexOf(_name) !== -1
                ? ''
                : '璧山区' === _name
                  ? _name.split('').join('\n')
                  : _name;
  ...
  const setMapToTip = (x, y) => {
    ...
    if (tipDom && mapConDom) {
      ...
      let pointMapInner;
      if (pointMap[1] <= pointOverTip[1]) {
        // 地图上的点高于提示框上方的点
        pointOverTip = [
          32 + 0.8 * remainWidth - 0.06 * conHeight,
          0.61 * conHeight - 76.8 - 0.06 * conHeight
        ];
        pointUnderTip = [
          32 + 0.8 * remainWidth + 0.18 * conHeight,
          conHeight - 76.8 + 0.06 * conHeight
        ];
        ...
        pointMapInner = `100% 0%`;
      } else if (pointMap[1] >= pointUnderTip[1]) {
        // 地图上的点低于提示框下方的点
        pointOverTip = [
          32 + 0.8 * remainWidth - 0.18 * conHeight,
          0.61 * conHeight - 76.8 - 0.06 * conHeight
...
const getHelpAreaData = async () => {
  ...
  if (Array.isArray(res))
    helpAreaData.value = res.map((item, index) => ({
      id: index,
      from: item.city,
      to: item.county
    }));
      ...
      children: Object.keys(data[firstItem]).map((secItem) => {
        return {
          ...
          children: data[firstItem][secItem].map((thirItem) => ({
            ...thirItem,
            bigscreenImg: thirItem.bigscreenImg ?? noImg
          }))
        };
      })
    });
...
const handleFirstItemClick = (item) => {
  ...
  if (Array.isArray(item.children) && item.children.length > 0) {
    ...
    if (
      Array.isArray(selectedSecItem.value.children) &&
      selectedSecItem.value.children.length > 0
    ) {
...
const handleImgChange = (val) => {
  ...
  const selectedImgIndex = selectedSecItem.value.children.findIndex(
    (item) => item.bigscreen === selectedThirItem.value.bigscreen
  );
  ...
  if (val > 0 && selectedImgIndex + 1 < selectedSecItem.value.children.length) {
    ...
    offsetPer.value =
      100 - ((selectedImgIndex + 1) * 100) / (selectedSecItem.value.children.length - 1);
  }
  ...
  <div class="w-full box-border relative overflow-y-auto h-100vh" id="homeContainer">
      ...
      <div class="container flex items-center p-5 space-x-1rem">
        ...
        <div class="text-white">
          ...
          <div class="text-.8rem relative left-.3rem text-[#f1f1f1c0]">
            Lu Yu Collaborative Rural Revitalization Village Digital Empowerment Project
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="container px-3rem box-border relative z-20">
        ...
        <div class="flex mt-10rem">
          <div class="outer-line p-1">
            ...
            >
              <div
                class="text-1rem flex justify-center items-center px-1rem pl-1.2rem relative z-30 tracking-widest"
              >
                系统介绍
              </div>
    ...
    <div class="w-full flex justify-center items-center h-100vh bg-2">
      <div class="container flex flex-row-reverse relative px-2rem box-border" id="mapContainer">
        ...
        <!-- 左侧弹框 -->
        <div
          class="flex flex-col py-[35px] box-border map-tip text-[0.67rem] lg:text-[1.33rem]"
          v-if="mapTipShow"
        >
          <div class="text-[1rem] lg:text-[2rem] text-[#48FF96] text-center">
            {{ mapTipData.county }}
          </div>
          <div
            :class="`mt-[0.5em] grid grid-cols-2 gap-2 text-white ${!mapTipData.city ? 'opacity-0' : 'opacity-100'}`"
          >
        ...
        <div
          class="absolute left-2rem top-[14%] text-white space-y-3 text-[1rem] lg:text-[1.4rem] xl:text-2.4rem"
        >
          <div>打造100个鲁渝协作乡村振兴示范镇</div>
          <div>打造100个鲁渝协作特色产业园区</div>
          <div>培育100个鲁渝协作品牌</div>
          <div class="text-[0.6rem] xl:text-1rem text-[#f1f1f1d0]">
            强化产业协作，实施“东产西移“，大力推动区域协同发展
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="container flex justify-center relative top-[-2rem] space-x-1">
        ...
        >
          ...
          <transition name="expand">
            <div
              v-show="selectedCardId === item.id"
              :class="`overflow-hidden ${selectedCardId === item.id ? 'linear-show' : ''}`"
            >
      ...
      <div class="container overflow-hidden px-[3rem] box-border">
        <div class="flex justify-between relative text-white py-5">
          ...
          <div class="flex space-x-2">
            <div
              @click="handleTurn(1)"
              class="swiper-button-next village-left cursor-pointer"
            ></div>
            <div
              @click="handleTurn(-1)"
              class="swiper-button-prev village-right cursor-pointer"
            ></div>
          </div>
        </div>
        <swiper
        ...
        >
          <swiper-slide v-for="item in countryBuildData" :key="item.id">
            <div
              class="w-21rem aspect-0.72 default-village-bg village-card-item box-border overflow-hidden"
            >
              ...
              <div
                class="w-full text-left py-1.9rem text-#fff text-1.3rem font-bold village-item-title box-border relative z-10"
              >
                {{ item.year }}年
              </div>
      ...
      <div class="w-full overflow-hidden pt-2rem container">
        ...
        >
          <swiper-slide v-for="item in supportIndustries" :key="item.id">
            <div
              class="w-full bg-white h-20rem overflow-hidden rounded-md mb-4rem flex flex-col items-center shadow-md"
            >
    ...
    <div class="flex justify-center items-center bg-6">
      <div class="container">
        ...
        <div class="flex">
          <div
            class="w-20rem p-2rem py-3.5rem box-border space-y-2rem"
            style="border-right: 1px solid #f1f1f130"
          >
            <div
          ...
          <div class="grow box-border p-2rem py-3rem pb-1rem">
            <div class="flex justify-between items-center">
              <div class="text-white text-1.7rem">
                {{ selectedSecIndustry.label }} - {{ selectedSecItem.label }} ({{
                  selectedSecItem?.children?.length
                }})
              </div>
              <div
                class="industry-btn w-3rem h-3rem cursor-pointer"
                @click="handleNextItem(1)"
              ></div>
            </div>
            <div class="flex justify-center items-center h-58vh">
              <div class="h-22rem aspect-1.68 tv-bg p-2.3rem box-border relative scale-130">
                ...
                />
                <div
                  class="absolute w-full h-2rem left-0 bottom-[3rem] flex justify-center items-center space-x-2rem"
                >
                  <div
                    class="left-btn w-2rem h-2rem cursor-pointer"
                    @click="handleImgChange(-1)"
                  ></div>
                  <div
                    class="right-btn w-2rem h-2rem cursor-pointer"
                    @click="handleImgChange(1)"
                  ></div>
                </div>
            ...
            <div class="w-full h-[10rem] bottom-[-2rem] flex justify-center">
              <div class="flex flex-col justify-center items-center w-25rem">
                <div class="flex justify-center h-20% text-center text-#fff text-1.2rem">
                  {{ selectedSecItem.label }}
                </div>
                <div
                  class="flex justify-center h-80% w-23rem semicircule-bg2 overflow-hidden"
                  @click="handleNextItem(-1)"
                >
        ...
        <div class="flex flex-col space-y-2">
          ...
          <div class="bg-white p-3 w-20rem box-border">
            <div class="line-clamp-1">
              中共重庆史农村工作实施乡村振兴战略领导小组关于2024年东西…
            </div>
            <div class="text-.7rem mt-1 pl-1 text-[#666] line-clamp-3">
              中共中央总书记、国家主席、中央军委主席习近平近日在重庆考察时强调，重庆要对标新时代新征程党的中心任务和党中央赋予的使命，充分发挥比较优势、后发优势，进一步全面深化改革开放，主动服务和融入新发…
            </div>
          </div>
        </div>
        <div class="flex flex-col space-y-2">
...
.village-card-item:hover {
  ...
  .village-animation-bg {
    animation: village-animation 0.4s linear forwards;
  }
}

.village-card-item {
  .village-animation-bg {
    animation: village-animation-reverse 0.4s linear forwards;
  }
}
...
.linear-show {
  animation: opacityIn 0.7s ease forwards;
}
/****************************** 第二页地图 start ******************************/
.map-tip {
  max-height: 39%;
