...
const handleCertify = async (id: number) => {
  try {
    // 删除的二次确认
    await message.confirm('请确认是否提交认证？');
    // 发起删除
    await ProductApi.certifyProduct(id, '待认证');
  ...
  >
    <div class="w-full flex justify-between items-center">
      <div class="flex items-center">
        <!-- 一级标题名字 -->
        <h1 class="m-0 text-[#333] font-bold text-[18px]">产品认证</h1>
    ...
    <div class="w-full mt-[16px]">
      ...
      <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
        ...
        <el-table-column
          ...
          width="180px"
        />
        <el-table-column label="操作" align="center" fixed="right" min-width="158px">
          <template #default="scope">
            <div class="flex items-center justify-center">
              ...
              <div
                v-if="scope.row.certifyStatus == '未认证'"
                class="mx-[12px] w-[1px] h-[24px] bg-[#e6e6e6]"
              ></div>
              <el-button
                link
                type="primary"
                @click="handleCertify(scope.row.id)"
                v-if="scope.row.certifyStatus == '未认证'"
