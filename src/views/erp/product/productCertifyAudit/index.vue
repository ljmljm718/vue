 ...
 * 通过认证操作
 */
const handleCertifyPass = async (id: number) => {
  try {
    // 删除的二次确认
    await message.confirm('请确认是否通过认证？');
    // 发起删除
    await ProductApi.certifyProduct(id, '已认证');
    message.success('通过认证成功！');
    // 刷新列表
    await getList();
  } catch {}
};
const handleCertifyBack = async (id: number) => {
  try {
    // 删除的二次确认
    await message.confirm('请确认是否退回认证？');
    // 发起删除
    await ProductApi.certifyProduct(id, '未认证');
    message.success('退回认证成功！');
    // 刷新列表
    await getList();
  } catch {}
};
  ...
  >
    <div class="w-full flex justify-between items-center">
      <div class="flex items-center">
        <!-- 一级标题名字 -->
        <h1 class="m-0 text-[#333] font-bold text-[18px]">产品认证审核</h1>
      ...
      <el-table v-loading="loading" :data="list" :stripe="true" :show-overflow-tooltip="true">
        ...
        <el-table-column label="认证状态" align="center" prop="certifyStatus">
          <template #default="scope">
            <dict-tag
              :type="DICT_TYPE.KAIZHOU_PRODUCT_CERTIFY_STATUS"
              :value="scope.row.certifyStatus"
            />
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template #default="scope">
            <div class="flex items-center justify-center">
              ...
              <el-button
                link
                type="primary"
                @click="handleCertifyPass(scope.row.id)"
                v-if="scope.row.certifyStatus == '待认证'"
                v-hasPermi="['erp:product:certify']"
              >
                通过
              </el-button>
              ...
              <el-button
                link
                type="primary"
                @click="handleCertifyBack(scope.row.id)"
                v-if="scope.row.certifyStatus == '待认证'"
                v-hasPermi="['erp:product:certify']"
              >
                退回
