...
<!--          v-hasPermi="['kaizhou:basic-scan-code:create']"-->
<!--        >-->
...
<!--            v-hasPermi="['kaizhou:basic-scan-code:update']"-->
<!--          >-->
<!--            编辑-->
<!--          </el-button>-->
          <el-button
            ...
            v-hasPermi="['kaizhou:basic-scan-code:delete']"
          >
            删除
          </el-button>
        </template>
  ...
  <!-- 表单弹窗：添加/修改 -->
  <BasicScanCodeForm ref="formRef" @success="getList" />
</template>
...
import BasicScanCodeForm from './BasicScanCodeForm.vue'
...
const getList = async () => {
  loading.value = true
  try {
    const data = await BasicScanCodeApi.getBasicScanCodePage(queryParams)
...
const handleDelete = async (id: number) => {
  try {
    ...
    // 发起删除
    await BasicScanCodeApi.deleteBasicScanCode(id)
...
const handleExport = async () => {
  try {
    ...
    const data = await BasicScanCodeApi.exportBasicScanCode(queryParams)
    download.excel(data, '扫码记录.xls')
  } catch {
  } finally {
