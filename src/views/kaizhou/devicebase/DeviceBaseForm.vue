...
const open = async (type: string, id?: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await DeviceBaseApi.getDeviceBase(id)
    } finally {
...
const submitForm = async () => {
  ...
  formLoading.value = true
  try {
    const data = formData.value as unknown as DeviceBaseVO
    if (formType.value === 'create') {
      await DeviceBaseApi.createDeviceBase(data)
      message.success(t('common.createSuccess'))
    } else {
      await DeviceBaseApi.updateDeviceBase(data)
