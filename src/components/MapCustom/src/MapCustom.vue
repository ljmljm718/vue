...
const handleSearch = () => {
  ...
  axios
    .get('/tdCache/api/tdtmap/search', {
      params: { keyWord: keyword.value }
    })
    .then(({ data }) => {
      console.log('🚀 ~ handleSearch ~ data:', data);
      if (Array.isArray(data))
        searchList.value = [...localSuggests.map((item) => item.meta), ...data];
    });
};
...
const addHTMLMarker = (content: string, iconSize: number[], latlng: number[]) => {
  ...
  const markerIcon = L.divIcon({
    html: content,
    className: 'div-icon-class',
    iconSize: iconSize //marker宽
  });
...
const clearHTMLMarker = () => {
  if (!map) return;
  htmlMarkerMap.forEach((item, key) => {
    map.removeLayer(item);
    htmlMarkerMap.delete(key);
  });
};
...
<template>
  <div class="w-full h-full relative">
    <div class="w-full h-full" :id="`mapIns_${componentID}`"></div>
    <div
